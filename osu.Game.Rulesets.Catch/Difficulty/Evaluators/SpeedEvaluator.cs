...
namespace osu.Game.Rulesets.Catch.Difficulty.Evaluators
{
    public class SpeedEvaluator
    {
        public static double EvaluateDifficultyOf(DifficultyHitObject current)
        {
            ...
            return 11.0 * (Math.Max(alternatingSpeed, Math.Max(1.1 * sameDirectionSpeed, 1.3 * delayedSameDirectionSpeed)) + 0.0 * delayedSameDirectionSpeed) * Math.Pow(((CatchDifficultyHitObject)current).MovementData.ActionProbability, 2);
